...
private val REGEX = "(?s)(<dependencyManagement>.+?<dependencies>)(.+?)(</dependencies>.+?</dependencyManagement>)".toRegex()
...
fun Project.fixMavenDependencyManagement() {
    tasks.withType(GenerateMavenPom::class.java).all { generateMavenPom ->
        generateMavenPom.doLast {
            ...
            val updatedPomXmlText = REGEX.find(pomXmlText)
                ?.let { matcher ->
                    val firstDeps = matcher.groups[2]!!.value
                    REGEX.replaceFirst(pomXmlText, "")
                        .let { REGEX.replaceFirst(it, "$1$2$firstDeps$3")  }
                }
                ?: pomXmlText
